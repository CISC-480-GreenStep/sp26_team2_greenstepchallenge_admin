...
export default function ComparisonMode({ stats, challenges, selectedChallengeIds }) {
  ...
  const barData = selectedChallenges.map((c) => {
    const cPart = stats.rawParticipation
      ? stats.rawParticipation.filter((p) => p.challengeId === c.id)
      : [];
      ...
      <Grid container spacing={3}>
          ...
          >
            <CardContent sx={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>
              <Typography variant="h6" gutterBottom>
                Relative Engagement Trend
              </Typography>
              <Box sx={{ width: "100%", flexGrow: 1, minHeight: 0 }}>
                <ResponsiveContainer
                  key={selectedChallengeIds.join("-")}
                  width="100%"
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <CardContent sx={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>
              <Typography variant="h6" gutterBottom>
                Total Participation & Points Comparison
              </Typography>
              <Box sx={{ width: "100%", flexGrow: 1, minHeight: 0 }}>
                <ResponsiveContainer
                  key={selectedChallengeIds.join("-")}
              ...
              <TableBody>
                <TableRow>
                  ...
                  {selectedChallenges.map((c, idx) => (
                    <TableCell key={c.id} align="right">
                      {formatNumber(barData[idx]?.participation || 0)}
                    </TableCell>
                  ))}
                </TableRow>
                <TableRow>
                  ...
                  {selectedChallenges.map((c, idx) => (
                    <TableCell key={c.id} align="right">
                      {formatNumber(barData[idx]?.points || 0)}
                    </TableCell>
                  ))}
                </TableRow>
                <TableRow>
