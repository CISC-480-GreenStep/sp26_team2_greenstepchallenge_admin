...
import {
  Drawer, Box, Typography, IconButton, TextField, Switch,
  Chip, Button, Stack, Tooltip, InputAdornment, alpha,
...
export default function WidgetCatalog({ 
  open, onClose, visible, onToggle, onApplyPreset, onReset,
  challenges = [], selectedChallengeIds = [], onToggleChallenge, onClearFilters,
  isComparisonModeActive, setIsComparisonModeActive
}) {
  ...
  const categories = Object.entries(WIDGET_CATEGORIES).sort(
    ([, a], [, b]) => a.order - b.order,
  );
  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: 2, py: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'action.hover' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Typography variant="subtitle2" fontWeight={700}>Filter Data by Challenge</Typography>
          {selectedChallengeIds.length > 0 && (
            <Button size="small" onClick={onClearFilters} sx={{ fontSize: '0.7rem' }}>Clear</Button>
      )}
  </Box>
  <Stack spacing={0.5} sx={{ maxHeight: 200, overflowY: 'auto' }}>
    {challenges.map((c) => (
      <Box 
        ...
        sx={{ display: 'flex', alignItems: 'center', p: 0.5, cursor: 'pointer', borderRadius: 1, '&:hover': { bgcolor: 'background.paper' } }}
      >
        <Switch size="small" checked={selectedChallengeIds.includes(c.id)} color="success" />
        <Typography variant="body2" sx={{ flex: 1, ml: 1 }}>{c.name}</Typography>
        {c.status === 'Upcoming' && (
          <Chip label="Upcoming" size="small" color="info" variant="outlined" sx={{ height: 16, fontSize: '0.6rem', mr: 0.5 }} />
        )}
        {(c.status === 'Completed' || c.status === 'Archived') && (
          <Chip label="Ended" size="small" variant="outlined" sx={{ height: 16, fontSize: '0.6rem' }} />
        )}
      </Box>
    ))}
  </Stack>
  ...
  <Tooltip title="Select 2 or more challenges to enable Comparison Mode" arrow placement="top">
    <Box>
      <FormControlLabel
        ...
        label={<Typography variant="body2" fontWeight={600}>Comparison Mode</Typography>}
      />
    </Box>
  </Tooltip>
