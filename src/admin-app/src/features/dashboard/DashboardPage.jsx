  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const totalPoints = filteredParticipation.reduce((sum, p) => {
          // FIXED
        ...
        const challengeStatusData = Object.entries(statusCounts).map(([name, value]) => ({
          name,
          value,
        }));
        ...
        const pointsDistributionData = Object.entries(buckets).map(([range, count]) => ({
          range,
          count,
        }));
