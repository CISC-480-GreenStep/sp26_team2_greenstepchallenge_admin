...
- **Reports** — `/reports` page (filter by challenge and date range, category breakdown chart, full participation table with clickable user/challenge links, CSV export); the **Participation Report** widget on the dashboard now mirrors the same filterable table + CSV export so the standalone page can be retired in a follow-up (issue #40)
...
- **Reports as dashboard widget (issue #40, slice 1)** — the filterable cross-challenge participation table from the legacy `/reports` page now also exists as a draggable dashboard widget under a new **Reports & Analysis** widget category. Drives toward folding `/reports` into the customizable dashboard so the standalone tab can be retired in a follow-up. Adds a pure aggregator `buildEnrichedParticipation` in `features/dashboard/hooks/aggregations.js` that pre-joins user / challenge / action / category names onto every row so the widget owns only its filter UI. Widget count: 22 → 23. The `/reports` route is intentionally left intact for this slice.
- **Categories migrated to Postgres (PR #66)** — admins can now create custom action categories from the Challenge and Template forms; the previous hardcoded six-category list (`General Sustainability`, `Food`, `Water`, `Energy`, `Transportation`, `Consumption & Waste`) is seeded into a new `categories` table via migration `013_create_categories.sql`. New service module `data/api/categories.js`. The `category` column on `actions` / `challenges` is still a string (storing the name) — FK normalization is tracked as a follow-up.
- **Repo housekeeping** — closed out the v0.9.0 consolidation tracker (issue #55): `Eli-feature-customizable-dashboard` and `feature/auth-magic-links` branches deleted; SuperAdmin permanent-delete rebuild tracked in issue #67.
